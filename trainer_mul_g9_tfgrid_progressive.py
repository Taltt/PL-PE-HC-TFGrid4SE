...
class Trainer:
    ...
    def _train_epoch(self, epoch):
        ...
        for step, data in enumerate(self.train_dataloader, 1):
            mixture,mix_1,mix_2,mix_3,mix_4,target = data
            mixture = mixture.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_1 = mix_1.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_2 = mix_2.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_3 = mix_3.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_4 = mix_4.to(torch.float32).to(self.device)                            # [B,N,M]
            ...
            loss = self.loss_func(esti_tagt[0],mix_1,mix_2,mix_3,mix_4,target)#+ self.loss_func(esti_nois, noise)
    ...
    def _validation_epoch(self, epoch):
        ...
        for step, data in enumerate(self.validation_dataloader, 1):
            mix,mix_1,mix_2,mix_3,mix_4,tgt,name = data
            mix = mix.to(torch.float32).to(self.device)  
            mix_1 = mix_1.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_2 = mix_2.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_3 = mix_3.to(torch.float32).to(self.device)                            # [B,N,M]
            mix_4 = mix_4.to(torch.float32).to(self.device)                                   # 
            ...
            loss = self.loss_func(esti_tagt[0],mix_1,mix_2,mix_3,mix_4,tgt)# + self.loss_func(esti_nois, noise)
            total_loss += loss.item()
